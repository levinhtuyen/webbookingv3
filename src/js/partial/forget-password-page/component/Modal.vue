  ...
  >
    ...
    <template slot="body">
      <div class="filter-modal__body">
        ...
        <div v-if="errorModal">
            <div class="error-modal" v-for="(error, index) in errorModal" :key="index">
                <p v-if="error.code == 20">{{ $t('forget_password.verify_code_incorrect') }}</p>
                <p v-if="error.code == 19">{{ $t('forget_password.verify_code_expired') }}</p>
            </div>
        </div>
