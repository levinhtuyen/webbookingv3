class handleTimeout {
  ...
  init(
    ...
    if(!asyncTimeoutName) {
      ...
      this.timeoutHandlerList.some(function(asyncHandlerItem, idx) {
        if(asyncHandlerItem.name === asyncTimeoutName) {
          isExist = true;

          if(typeof asyncHandlerItem.timeout !== 'function') {
            asyncTimeoutIdx = idx;
          }
    ...
    asyncTimeoutIdx = (
      asyncTimeoutIdx === null
    ) ? this.timeoutHandlerList.length : asyncTimeoutIdx;
